...
export default function GuitarNeck({
  ...
  function hasMarker(stringIndex: number, fretIndex: number): boolean {
    return markers.some(
      (m) => m.string === stringIndex && m.fret === fretIndex,
    )
  }

  function handleTap(stringIndex: number, fretIndex: number) {
    onMarkerToggle?.({ string: stringIndex, fret: fretIndex })
  }
  ...
  return (
    ...
    >
      {/* Nut (fret 0) - visually distinct thick bar */}
      <rect
      ...
      {/* Tap targets for fret 0 (nut / open string) */}
      ...
      {Array.from({ length: fretCount }, (_, fretIdx) => fretIdx + 1).map(
        (fret) =>
          Array.from({ length: stringCount }, (_, s) => {
            const cx = stringX(s)
            const cy = fretY(fret) - fretHeight / 2
