...
export default function Dashboard() {
  ...
  const ownedProjects = projects.filter((p) => p.my_role === 'owner')
  const sharedProjects = projects.filter((p) => p.my_role !== 'owner')

  function renderProjectCard(project: Project) {
    const isShared = project.my_role !== 'owner'
    return (
      <div
        key={project.id}
        ...
        <div className="space-y-6">
          ...
          {sharedProjects.length > 0 && (
            <div>
              ...
              <div className="space-y-3">
                {sharedProjects.map((project) => renderProjectCard(project))}
              </div>
            </div>
          )}
        </div>
